import 'package:kendedes_mobile/classes/providers/local_db/move_db_provider.dart';
...
class MoveDbRepository {
  static final MoveDbRepository _instance = MoveDbRepository._internal();
  factory MoveDbRepository() => _instance;

  MoveDbRepository._internal();

  late MoveDbProvider _moveDbProvider;
  ...
  Future<void> init() async {
    ...
    _initialized = true;
    _moveDbProvider = MoveDbProvider();
    await _moveDbProvider.init();
  ...
  Future<bool> createSlsWithBusiness(SlsWithBusiness item) async {
    ...
    if (!pairExists) {
      await _moveDbProvider.createSlsWithBusiness(item.toJson());
    }
    return !pairExists;
  }
  ...
  Future<void> updateSlsWithBusiness(SlsWithBusiness item) async {
    ...
    await _moveDbProvider.updateSlsWithBusiness(item.id, data);
  }

  Future<void> deleteSlsWithBusiness(String id) async {
    await _moveDbProvider.deleteSlsWithBusiness(id);
  }

  Future<List<SlsWithBusiness>> getSlsWithBusinessList({
    required String currentUserId,
  }) async {
    final rows = await _moveDbProvider.getSlsWithBusinessList(
  ...
  Future<void> insertBusinessesDataBatch(
    ...
    final existingBusiness =
        existingProjects.isEmpty
            ? <TagData>[]
            : await getBusinessesByMoveProjects(
    ...
    for (int i = 0; i < businesses.length; i += chunkSize) {
      ...
      await _moveDbProvider.insertBusinessesDataBatch(mappedChunk);
    }
  }
  ...
  Future<void> insertProjectBatch(List<Project> projects) async {
    ...
    await _moveDbProvider.insertProjectsBatch(dataList);
  }
  ...
  Future<List<User>> getAllUsers() async {
    final maps = await _moveDbProvider.getAllUsers();
    return maps.map((map) {
      return User.fromJson(map);
    }).toList();
  }

  // insert user by calling moveDbProvider.insertUsersBatch
  Future<void> insertUsersBatch(List<User> users, {int chunkSize = 500}) async {
    ...
    for (int i = 0; i < users.length; i += chunkSize) {
      ...
      await _moveDbProvider.insertUsersBatch(mappedChunk);
    }
  }
  ...
  Future<List<TagData>> getBusinessesByMoveProjects(
    List<String> projectIds,
    String userId,
  ) async {
    final businessMaps = await _moveDbProvider.getBusinessesByMoveProjects(
      projectIds,
    );
  ...
  Future<bool> deleteBusinessesBySlsId(String slsId, String userId) async {
    ...
    return await _moveDbProvider.deleteBusinessesBySlsId(slsId, projectIds);
  }
  ...
  Future<bool> deleteBusinessesByIds(
    ...
    return await _moveDbProvider.deleteBusinessesByIds(
      businessIds,
      projectIds,
    );
  }
  ...
  Future<int?> getSlsWithBusinessCountBySlsId(
    ...
    return _moveDbProvider.getSlsWithBusinessCountBySlsId(slsId, userId);
  }
  ...
  Future<int?> getBusinessCountBySlsId(
    ...
    return _moveDbProvider.getBusinessCountBySlsId(slsId, projectIds);
  }

  Future<bool> hasPolygonBySlsId(String slsId, String userId) async {
    return _moveDbProvider.hasPolygonBySlsId(slsId, userId);
  }
